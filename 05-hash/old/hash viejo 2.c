...
#include <stdint.h>
#include "hash.h"
#define TAM_INICIAL 79
#define FACTOR_REDIMENSION 0.2
int REDIMENSION_GLOBAL = 0;

int primos [] = {163,457,1009,2503,6007,15013,50021,120011,350003,700001,1500007,5000011};
...
bool llenar_tabla_hash(hash_campo_t* tabla,size_t largo) {
  ...
  bool ok = true;
  for(; i <= largo;i++) {
    hash_campo_t* campo = crear_hash_campo();
    if(!campo) {
      ok = false;
      break;
    }
  ...
  if(!ok) { //liberar memoria en caso de error durante la asignacion
    for(int j = 0;j<i;j++) free(&tabla[j]);
    return false;
  }
  return true;
}
...
bool redimensionar_tabla(hash_t* hash) {
  size_t carga = hash->carga;
  size_t largo = hash->largo;
  float redim = (float) carga/ (float) largo;
  if( redim > FACTOR_REDIMENSION) {
    ...
    if(!llenar_tabla_hash(nueva_tabla,primos[REDIMENSION_GLOBAL])) {
      free(nueva_tabla);
      return false;
    }
...
hash_t* hash_crear(hash_destruir_dato_t destruir_dato) {
  ...
  bool llenado = llenar_tabla_hash(hash->tabla,TAM_INICIAL);
  if(!llenado) {
    free(tabla);
    free(hash);
    return NULL;
