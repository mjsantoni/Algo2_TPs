...
/*
	Posiciona el iterador en la fecha deseada, apilando mientras busca, para luego poder iterar
	normalmente. Tarda tiempo logaritmico en encontrar la fecha ya que busca en el abb.
*/

abb_nodo_t* apilar_buscar_nodo(abb_nodo_t* nodo,const char* clave,const abb_t* abb,pila_t* pila) {
	if(!nodo) return NULL;
	abb_comparar_clave_t cmp = abb->cmp;
	pila_apilar(pila, nodo);	
	...
	if(cmp(nodo->clave,clave) > 0) { //Tiene que estar a la izquierda de este nodo.
		...
		return apilar_buscar_nodo(nodo->izq,clave,abb,pila); //Busco a la izquierda.
	}
	if(!nodo->der) return nodo;
	return apilar_buscar_nodo(nodo->der,clave,abb,pila); //Busco a la derecha
}

/*
	Avanza el iterador en una posicion, teniendo en cuenta que al crear el iterador
	se apilaron cosas que podrían no estar en el rango de fechas.
	Si se llega a la fecha tope, se vacía la pila para que pare la iteración.
	Funciona igual que el iterador común.
*/
bool abb_iter_in_avanzar_2_punto_0(abb_iter_t *iter,char* inicio,char* fin,const abb_t* abb) {
	if(abb_iter_in_al_final(iter)) return false;
	abb_comparar_clave_t cmp = abb->cmp;
	abb_nodo_t* desapilado = pila_desapilar(iter->pila);
	bool entro_while = false;

	while( (cmp(desapilado->clave,inicio) < 0) && !pila_esta_vacia(iter->pila)) {
		entro_while = true;
		abb_nodo_t* tope_cmp = (abb_nodo_t*) pila_ver_tope(iter->pila);
		if(cmp(tope_cmp->clave,inicio) >= 0) break;
		else desapilado = pila_desapilar(iter->pila);
	}
	if(!entro_while) {
		if(desapilado->der) _agregar_a_pila(desapilado->der,iter->pila);
	}
	iter->nodo = (abb_nodo_t*) pila_ver_tope(iter->pila);
	abb_nodo_t* tope = (abb_nodo_t*) pila_ver_tope(iter->pila);
	if(tope && cmp(tope->clave,fin) > 0) {
...
/*
	Crea el iterador modificado para que comience la iteración desde la fecha pedida
	y asi cumplir con los tiempos requeridos.
*/
abb_iter_t* abb_iter_2_punto_0 (abb_t* arbol, const char* inicio) {
	if(!arbol) return NULL;
	abb_iter_t* iter = calloc(1,sizeof(abb_iter_t));
	if(!iter) return NULL;
	pila_t* pila = pila_crear();
	...
	iter->pila = pila;
	apilar_buscar_nodo(arbol->raiz,inicio,arbol,iter->pila);
	iter->nodo = (abb_nodo_t*) pila_ver_tope(iter->pila);
	return iter;
}
