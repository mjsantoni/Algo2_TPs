...
void pruebas_encolar() {
	...
	bool ok = true;
    //Chequea que siempre salga uno mas grande que el otro.
  	while(!heap_esta_vacio(heap)) {
  		void* uno = heap_desencolar(heap);
  		void* dos = heap_desencolar(heap);
  		if(!dos) break;

  		if(comparador(uno,dos) < 0) {
  			ok = false;
  			break;
  		}
    }
    print_test("Se respesto el orden", ok);
...
/////////////////////////////////////////////////////////////////////
int largos_cadenas(const void* uno,const void* dos) {
	const char* u = uno;
	const char* d = dos;
	size_t resul = strlen(u) - strlen(d);
	return (int) resul;
}
...
void prueba_heapify() {
	...
	vector[0] = "CORTE TOTAL";
	vector[1] = "LLEGO TRUMP";
	vector[2] = "PLAZA DE MAYO CERRADA";
	vector[3] = "G20";
	vector[4] = "GENDARMERIA NO ME DEJA PASAR";
	vector[5] = "COSTA SALGUERO";
	vector[6] = "TRANSITO EN TODA LA CIUDAD DE BUENOS AIRES";
	void* v0 = &vector[0];
	void* v1 = &vector[1];
	void* v2 = &vector[2];
	void* v3 = &vector[3];
    ...
    heap_t* heap = heap_crear_arr(puntero,7,largos_cadenas);
