...
#Se comento vacacions ya que la prueba de Minneapolis 12 no termina nunca, si se prueban las de la pagina no hay ningun problema
#El tema es que la poda es muy basica y no se puede hacer mucha mas que esa, ya que es casi un bruteforce.
operaciones = ["camino_mas","camino_escalas",
	"centralidad","centralidad_aprox",
	"pagerank","nueva_aerolinea",
	"recorrer_mundo_aprox","recorrer_mundo",#"vacaciones",
...
def ejecutor_de_comandos(comando,parametros,grafo,datos_aeropuertos,aeropuertos_de_ciudades,camino_kml):
	...
	if comando == "recorrer_mundo_aprox": return recorrer_mundo_aprox(grafo,aeropuertos_de_ciudades,parametros[0],False)
...
def _recorrer_mundo_aprox(grafo_prim,origen):
	recorrido_rapido,tiempo_rapido = repo.dfs_recorrer_mundo_aprox(grafo_prim,origen)
...
def recorrer_mundo_aprox(grafo,aeropuertos_de_ciudades,origen,cota_max):
	grafo_prim = repo.prim_no_dirigido(grafo,1)	
	mejor_recorrido = []	
	mejor_tiempo = float("inf")

	for aerop_origen in aeropuertos_de_ciudades[origen]:
		recorrido,tiempo = 	_recorrer_mundo_aprox(grafo_prim,aerop_origen)
		if tiempo < mejor_tiempo:
			mejor_tiempo = tiempo
			mejor_recorrido = recorrido
	if (cota_max): #Si cota_max es True solo se quiere el camino y el tiempo para usarlos como cota
		return mejor_recorrido,mejor_tiempo
		
	#Sino es que se queire imprimir el aprox
...
def recorrer_mundo(grafo,aeropuertos_de_ciudades,origen):
	mejor_recorrido,mejor_tiempo = recorrer_mundo_aprox(grafo,aeropuertos_de_ciudades,origen,True)

	for aerop_origen in aeropuertos_de_ciudades[origen]:
		recorrido,tiempo = 	repo.recorrer_mundo_preciso(grafo,aerop_origen,mejor_tiempo)
		if tiempo < mejor_tiempo:
			mejor_tiempo = tiempo
			mejor_recorrido = recorrido
...
def vacaciones(grafo,aeropuertos_de_ciudades,origen,cantidad):
	...
	for aeropuerto_origen in aeropuertos_de_ciudades[origen]:
		...
		repo.vacaciones(grafo,aeropuerto_origen,cantidad,1,aeropuerto_origen,viaje,visitados)
