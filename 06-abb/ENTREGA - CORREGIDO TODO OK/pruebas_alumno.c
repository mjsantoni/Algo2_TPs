...
int invertidos(const char* palabra1,const char* palabra2) {
	if(strcmp(palabra1,palabra2) < 0) return 1;
	else if(strcmp(palabra1,palabra2) > 0) return -1;
	else return 0;
}

static void prueba_abb_insertar() {

    abb_t* abb = abb_crear(invertidos,NULL);
...
/* Provisto por https://gist.github.com/fbarrios/11d71b3a7e2c0003f77c41a056718a75 */
...
static void prueba_abb_iterar_volumen(size_t largo) {
    ...
    for(int j = 0;j<largo;j++) free(claves[j]);
    abb_iter_in_destruir(iter);
    abb_destruir(abb);
}
